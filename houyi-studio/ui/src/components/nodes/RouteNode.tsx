...
export const RouteNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const statusConfig: Record<string, { bg: string; border: string; icon: string; text: string }> = {
    ...
    running: { bg: 'bg-orange-100', border: 'border-orange-500', icon: '▶', text: 'text-orange-700' },
    ...
    <div className={`px-4 py-2 shadow-lg rounded-md border-2 ${config.bg} ${config.border} ${
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <div className={`text-sm font-bold ${config.text}`}>{data.label || 'Route'}</div>
          <div className="text-xs text-gray-600">Route Node</div>
        </div>
