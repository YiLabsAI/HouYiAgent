...
export const LLMNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  // Get status from node_executions (more reliable than data.status)
  ...
  const statusConfig: Record<string, { bg: string; border: string; icon: string; text: string }> = {
    ...
    running: { bg: 'bg-blue-100', border: 'border-blue-500', icon: '▶', text: 'text-blue-700' },
    ...
    <div className={`px-4 py-2 shadow-lg rounded-md border-2 ${config.bg} ${config.border} ${
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <div className={`text-sm font-bold ${config.text}`}>{data.label || 'LLM'}</div>
          <div className="text-xs text-gray-600">LLM Node</div>
        </div>
