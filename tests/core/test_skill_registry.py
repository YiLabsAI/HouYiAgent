...
class TestSkillRegistryManifestLoading:
    """Test SkillRegistry manifest and file loading methods."""
...
""")
        ...
        registry = SkillRegistry()
        name = registry.register_from_skill_file(skill_file)

        assert name == "test-file-skill"
        assert registry.get("test-file-skill") is not None

    def test_register_from_skill_file_not_found(self, tmp_path: Path) -> None:
        """Test error when skill file doesn't exist."""
        registry = SkillRegistry()

        with pytest.raises(FileNotFoundError):
            registry.register_from_skill_file(tmp_path / "nonexistent.md")

    def test_register_from_directory(self, tmp_path: Path) -> None:
        """Test discovering skills from directory."""
        # Create skill files in subdirectories
...
""")
        ...
        assert "dir-skill-1" in registered
        assert "dir-skill-2" in registered
        assert registry.get("dir-skill-1") is not None
        assert registry.get("dir-skill-2") is not None

    def test_register_from_directory_non_recursive(self, tmp_path: Path) -> None:
...
""")
        ...
        # Create skill in subdirectory (should not be found)
...
""")
        ...
        registry = SkillRegistry()
        registered = registry.register_from_directory(tmp_path, recursive=False)

        assert len(registered) == 1
        assert "root-skill" in registered
        assert registry.get("sub-skill") is None

    def test_register_from_manifest(self, tmp_path: Path) -> None:
        """Test loading skills from simpleskill.json manifest."""
        # Create skill file
...
""")
        ...
        # Create manifest
        manifest_file = tmp_path / "simpleskill.json"
        manifest_file.write_text("""{
    "id": "test.manifest",
    "version": "1.0.0",
    "name": "Test Manifest",
    "description": "Test manifest for loading skills",
    "contributions": {
        "skills": [
            {
                "id": "manifest-skill",
        ...
        registry = SkillRegistry()
        registered = registry.register_from_manifest(manifest_file)

        assert len(registered) == 1
        assert "manifest-skill" in registered
        assert registry.get("manifest-skill") is not None

    def test_register_from_manifest_not_found(self, tmp_path: Path) -> None:
        """Test error when manifest doesn't exist."""
        registry = SkillRegistry()

        with pytest.raises(FileNotFoundError):
            registry.register_from_manifest(tmp_path / "nonexistent.json")

    def test_get_manifest(self, tmp_path: Path) -> None:
        """Test loading manifest without registering skills."""
        manifest_file = tmp_path / "simpleskill.json"
        manifest_file.write_text("""{
    "id": "inspect.manifest",
    "version": "2.0.0",
    "name": "Inspect Manifest",
    "description": "Manifest for inspection"
}""")

        registry = SkillRegistry()
        manifest = registry.get_manifest(manifest_file)

        assert manifest.id == "inspect.manifest"
