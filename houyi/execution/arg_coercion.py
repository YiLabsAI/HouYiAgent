...
functions are registered in a global registry keyed by tool name.  The runner
calls ``coerce_args(tool_name, args, resolved_outputs)`` which dispatches to
the appropriate handler (or returns *args* unchanged).
"""
