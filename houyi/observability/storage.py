...
class SQLiteSpanStorage(SpanStorage):
    ...
    def _row_to_span(self, row: sqlite3.Row) -> SpanSchema:
        ...
        from houyi.observability.types import CostInfo, SpanEvent, TokenUsage
    ...
    def query(self, filter: SpanFilter) -> list[SpanSchema]:
        ...
        if filter.span_id:
            ...
            params.append(filter.span_id)
        if filter.parent_span_id:
            conditions.append("parent_id = ?")
            params.append(filter.parent_span_id)
        if filter.node_id:
            conditions.append("node_id = ?")
            params.append(filter.node_id)
        if filter.span_type:
            conditions.append("span_type = ?")
            params.append(filter.span_type.value)
        if filter.status:
            conditions.append("status = ?")
            params.append(filter.status)
        if filter.start_time_gte is not None:
            conditions.append("start_time >= ?")
            params.append(filter.start_time_gte)
        if filter.start_time_lte is not None:
            conditions.append("start_time <= ?")
            params.append(filter.start_time_lte)
        if filter.end_time_gte is not None:
            conditions.append("end_time >= ?")
            params.append(filter.end_time_gte)
        if filter.end_time_lte is not None:
            conditions.append("end_time <= ?")
            params.append(filter.end_time_lte)
