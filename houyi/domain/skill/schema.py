...
def _match_yaml_key_value(stripped: str) -> re.Match[str] | None:
    return re.match(r"^([a-zA-Z_-]+):\s*(.*)$", stripped)


def _match_yaml_list_item(stripped: str) -> re.Match[str] | None:
    return re.match(r"^-\s+(.*)$", stripped)


def _start_nested_yaml_block(key: str, indent: int) -> tuple[str, int, dict[str, Any]]:
    return key, indent, {}
    ...
    *,
    ...
    if indent == 0:
        if value:
            ...
            return None, 0, {}
        return _start_nested_yaml_block(key, indent)

    if current_key and indent > current_indent and value:
        nested_dict[key] = _parse_yaml_value(value)

    return current_key, current_indent, nested_dict


def _append_yaml_list_value(
    *,
    result: dict[str, Any],
    current_key: str | None,
    stripped: str,
    ...
    if not current_key:
        return
    list_match = _match_yaml_list_item(stripped)
    if not list_match:
        return
    ...
    if isinstance(bucket, list):
        bucket.append(_parse_yaml_value(value))


def _iter_hook_dicts(hook_list: Any) -> list[dict[str, Any]]:
    hook_items = hook_list if isinstance(hook_list, list) else [hook_list]
    return [hook for hook in hook_items if isinstance(hook, dict)]


def _resolve_hook_event(event_name: str) -> HookEvent | None:
    try:
...
def _append_nested_hooks(
    *,
    hooks: list[SkillHook],
    event: HookEvent,
    matcher: str | None,
    nested_hooks: list[Any],
    default_type: HookType,
    append_hook,
) -> bool:
    valid_nested_hooks = [nested for nested in nested_hooks if isinstance(nested, dict)]
    if not valid_nested_hooks:
        return False

    for nested in valid_nested_hooks:
        hook = append_hook(
            event=event,
            matcher=matcher,
            hook_dict=nested,
...
def _simple_yaml_parse(content: str) -> dict[str, Any]:
    ...
    for line in content.split("\n"):
        ...
        kv_match = _match_yaml_key_value(stripped)
        if kv_match:
            key = kv_match.group(1)
            value = kv_match.group(2).strip()

            current_key, current_indent, nested_dict = _assign_yaml_key_value(
                result=result,
                key=key,
                value=value,
                indent=indent,
        ...
        _append_yaml_list_value(result=result, current_key=current_key, stripped=stripped)
...
def parse_hooks_config(config: dict[str, Any]) -> list[SkillHook]:
    ...
    for event_name, hook_list in config.items():
        event = _resolve_hook_event(event_name)
        if not event:
            continue

        for hook_config in _iter_hook_dicts(hook_list):
            default_type = _hook_type_from(hook_config.get("type", "handler"))
            nested_hooks = hook_config.get("hooks")

            if isinstance(nested_hooks, list) and _append_nested_hooks(
                hooks=hooks,
                event=event,
                matcher=hook_config.get("matcher"),
                nested_hooks=nested_hooks,
                default_type=default_type,
                append_hook=_append_hook,
            ):
                continue
