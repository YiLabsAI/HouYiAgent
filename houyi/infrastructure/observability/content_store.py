...
class FileContentStore(ContentStore):
    ...
    def cleanup_old_content(self, before_timestamp: float) -> int:
        ...
        for shard_dir in self.base_path.iterdir():
            ...
            for trace_dir in shard_dir.iterdir():
                ...
                to_delete = self._collect_expired_content_ids(manifest, before_dt)
                count += self._delete_manifest_contents(trace_dir, manifest, to_delete)
                if to_delete:
                    self._save_manifest(trace_dir.name, manifest)
                self._cleanup_empty_trace_dir(trace_dir, manifest)

        return count

    @staticmethod
    def _collect_expired_content_ids(manifest: dict[str, Any], before_dt: datetime) -> list[str]:
        expired: list[str] = []
    ...
    def _cleanup_empty_trace_dir(self, trace_dir: Path, manifest: dict[str, Any]) -> None:
        if manifest.get("contents"):
            return
        try:
