...
class ConsoleExporter(Exporter):
    ...
    def export(self, span_data: dict[str, Any]) -> None:
        ...
        if self.verbose:
            logger.info("\n%s", "=" * 60)
            logger.info("Trace: %s", span_data["name"])
            logger.info("%s", "=" * 60)
            ...
            logger.info("%s %s (%.2fms)", status_icon, span_data["name"], duration_ms)
    ...
    def _print_span(self, span: dict, indent: int = 0) -> None:
        ...
        logger.info("%s📍 %s (%.2fms)", prefix, span["name"], duration_ms)

        if span["attributes"]:
            logger.info("%s   Attributes:", prefix)
            for key, value in span["attributes"].items():
                logger.info("%s     - %s: %s", prefix, key, value)

        if span["events"]:
            logger.info("%s   Events:", prefix)
            for event in span["events"]:
                logger.info("%s     - %s", prefix, event["name"])
...
class JaegerExporter(Exporter):
    ...
    def __init__(
        ...
        logger.info("✅ JaegerExporter initialized - %s", self.endpoint)
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=self.timeout) as response:
                if response.status == 200:
                    logger.info("✅ Exported %d spans to Jaeger", len(self.span_batch))
                else:
                    logger.warning("Jaeger export failed: %s", response.status)
        except urllib.error.URLError as e:
            logger.warning("Jaeger export error: %s", e)
...
class DatadogExporter(Exporter):
    ...
    def __init__(
        ...
        logger.info("✅ DatadogExporter initialized - %s", self.agent_url)
    ...
    def flush(self) -> None:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=self.timeout) as response:
                if response.status == 200:
                    logger.info("✅ Exported %d traces to Datadog", len(self.trace_batch))
                else:
                    logger.warning("Datadog export failed: %s", response.status)
        except urllib.error.URLError as e:
            logger.warning("Datadog export error: %s", e)
        except Exception as e:
            logger.warning("Datadog export error: %s", e)
        finally:
            self.trace_batch = []
...
class StorageExporter(Exporter):
    ...
    def export(self, span_data: dict[str, Any]) -> None:
        ...
        from houyi.infrastructure.observability.types import SpanSchema

        try:
            span = SpanSchema(**span_data)
            self.storage.save(span)
        except Exception as e:
            logger.warning("Storage export error: %s", e)
