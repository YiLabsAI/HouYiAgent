...
class RuntimeMode(str, Enum):
    ...
    TOOL = "tool"
    """Standard tool calling (Python callable bound as executor)."""

    SCRIPT = "script"
    """External script execution (shell command / subprocess)."""

    TEMPLATE = "template"
    """Prompt template only (no executor, LLM follows instructions)."""


class CapabilityTier(int, Enum):
    """Capability tier for a loaded skill.

    Ordered so that higher values represent more complete integration.
    """

    METADATA = 1
    """Discoverable: name, description, source are available."""

    SCHEMA = 2
    """Validatable: structured input/output schema present."""

    EXECUTABLE = 3
    """Executable: runtime contract resolved, executor bound."""
...
class RuntimeStatus(str, Enum):
    ...
    DEGRADED = "degraded"
    """Partially operational (e.g. has schema but no executor)."""

    UNAVAILABLE = "unavailable"
    """Not operational (missing executor and schema)."""
...
class RuntimeContract(BaseModel):
    ...
    def from_dict(cls, data: Any) -> RuntimeContract | None:
        """Parse a runtime dict from frontmatter.

        Returns ``None`` if *data* is not a dict (graceful degradation).
        Invalid ``mode`` values fall back to ``tool`` with the original
        value preserved in ``extra["original_mode"]``.
        """
        if data is None:
            return None
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize to a dict suitable for frontmatter or API response."""
