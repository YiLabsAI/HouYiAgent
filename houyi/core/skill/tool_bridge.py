...
def _simplify_property_schema(prop: dict[str, Any]) -> dict[str, Any]:
    """Drop noisy metadata and flatten nullable ``anyOf`` shapes."""
    result: dict[str, Any] = {}
...
def _simplify_schema(schema: dict[str, Any]) -> dict[str, Any]:
    """Normalize JSON schema for concise tool definitions."""
    cleaned = dict(schema)
...
def _schema_from_input_model(input_model: Any) -> dict[str, Any]:
    """Safely render input schema from Pydantic model-like objects."""
    if not input_model:
        return {}
...
def build_tool_definitions_for_skill(skill: Any) -> list[dict[str, Any]]:
    """Build OpenAI-format tool definitions from a skill-like object.

    Supports both:
    1. Multi-tool layout: ``skill.tools`` exists.
    2. Single-tool layout: tool fields live on the skill itself.
    """
...
class ToolBridge:
    ...
    def collect_skills(
        ...
        """Collect skills from registry with optional name filter."""
        if skill_filter:
            seen: set[int] = set()
            selected: list[Any] = []
    ...
    def collect_tool_schemas(
        ...
        """Collect OpenAI tool schemas with cache, relevance trim, and sorting."""
