...
async def pre_write_hook(context: HookContext) -> HookResult:
    ...
    try:
        ...
        if progress["remaining"] > 0:
            ...
            if remaining:
                output += "[Planning] Remaining subtasks:\n"
        ...
        logger.warning("Error in pre_write_hook: %s", e)
        return HookResult(success=True)
...
async def post_tool_hook(context: HookContext) -> HookResult:
    ...
    try:
        ...
        logger.warning("Error in post_tool_hook: %s", e)
        return HookResult(success=True)
...
async def stop_hook(context: HookContext) -> HookResult:
    ...
    try:
        ...
        logger.warning("Error in stop_hook: %s", e)
        return HookResult(success=True)
