...
run_check "Ruff" "ruff check . --fix"

# 3. Pylint - Deep code quality check
run_check "Pylint (source code)" "pylint houyi/ --rcfile=.pylintrc"

# 4. Type checking with MyPy (optional, can be slow)
# run_check "MyPy" "mypy houyi/ --ignore-missing-imports"

# 5. Run tests
run_check "Unit Tests" "pytest tests/ -v --tb=short -x"

# 6. Check test coverage
echo -e "${YELLOW}▶ Checking test coverage...${NC}"
pytest tests/ --cov=houyi --cov-report=term-missing --cov-fail-under=75 || {
